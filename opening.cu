...
int main()
{
  ...
  erosionImg<<<number_of_blocks, threads_per_block>>>(res_ero_gpu,clc_gpu, witdh, height,size_of_filter);
  cudaDeviceSynchronize();
  erosionCPU(res_ero_cpu,clc_cpu,witdh,height,size_of_filter);
  
  generateClc(res_ero_gpu,clc_gpu,size_of_filter,witdh,height);
  generateClc(res_ero_cpu,clc_cpu,size_of_filter,witdh,height);
  
  
  dilationImg<<<number_of_blocks, threads_per_block>>>(res,clc_gpu, witdh, height,size_of_filter);
  cudaDeviceSynchronize();
  dilationCPU(res_cpu,clc_cpu,witdh,height,size_of_filter);
